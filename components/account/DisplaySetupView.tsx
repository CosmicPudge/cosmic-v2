...
export default function DisplaySetupView({ deviceId }: { deviceId: string }) {
  ...
  const loadConnections = useCallback(async () => {
    ...
    try {
      ...
      const normalized = setupConnectionProviders.map((provider) => {
        const record = records.find((item) => item.provider?.toLowerCase() === provider.toLowerCase());
        const reconnectRequired = Boolean(record?.reconnectRequired);
        const connected = Boolean(record && record.status === "connected" && !reconnectRequired);
  ...
  const connectionLink = (provider: string) => provider === "Spotify" ? `/api/auth/spotify?returnTo=${encodeURIComponent(setupReturn)}` : provider === "Gmail" ? `/api/auth/google?returnTo=${encodeURIComponent(setupReturn)}` : `/account?returnTo=${encodeURIComponent(setupReturn)}`;
  ...
  const renderStep = () => {
    ...
    if (step === "connections") return <Panel title="CONNECT YOUR COSMIC"><p className="text-sm text-white/55">Optional account connections. You can continue without any of them.</p>{connectionLoadState === "loading" ? <p className="mt-4 rounded-xl border border-dashed border-white/10 px-3 py-4 text-sm text-white/45">Checking connection status…</p> : connectionLoadState === "error" ? <div className="mt-4 rounded-xl border border-amber-200/15 bg-amber-200/[.04] px-3 py-4 text-sm text-amber-50/75"><p>Connections couldn&apos;t be checked right now.</p><p className="mt-1 text-xs text-white/45">You can continue setup and connect services later.</p><button type="button" onClick={() => void loadConnections()} className="mt-3 rounded-lg border border-cyan-200/20 px-3 py-2 text-xs text-cyan-50">Retry</button></div> : <div className="mt-4 space-y-2">{providerRows.map((item) => <div key={item.provider} className="flex items-center justify-between gap-3 rounded-xl border border-white/10 px-3 py-3"><div><p className="text-sm font-medium text-white/85">{item.provider}</p><p className="text-xs text-white/45">{item.detail}</p></div>{item.connected ? <span className="text-sm text-emerald-200">✓ Connected</span> : item.configured ? <a href={connectionLink(item.provider)} className="rounded-lg border border-cyan-200/20 px-3 py-2 text-xs text-cyan-50">Connect</a> : <span className="text-xs text-white/45">Not configured</span>}</div>)}</div>}</Panel>;
