...
export default function DisplaySetupView({ deviceId }: { deviceId: string }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setProfile(next); setDisplay(next.display ?? null); setName(next.deviceName ?? ""); setTimezone(next.timezoneOverride ?? ""); setLocation(next.location ?? null); setNightPreview(next.nightDimPreview); setStatus("Connected to display");
  ...
  function displayTime(value?: string) { if (!value) return "Waiting for kiosk timezone"; try { return new Intl.DateTimeFormat("en-US", { timeZone: value, hour: "numeric", minute: "2-digit" }).format(new Date()); } catch { return "Time zone could not be detected"; } }
  ...
  const locationSource = current.location?.source === "detected" ? "Detected from kiosk" : current.location?.source === "manual" ? "Phone location" : current.location ? "Display location" : "Account default";
  ...
  return <main className="min-h-screen bg-[#030511] px-4 py-5 text-white sm:px-8"><div className="mx-auto w-full max-w-xl"><header><p className="text-[.65rem] uppercase tracking-[.3em] text-cyan-200/65">Cosmic OS · display setup</p><h1 className="mt-2 text-3xl font-semibold">Configure your display</h1><p className="mt-2 text-sm text-white/50">All setup controls stay on this phone. The display is a passive live preview.</p></header><p className="mt-4 rounded-xl border border-cyan-200/15 bg-cyan-200/[.05] px-3 py-2 text-xs text-cyan-50/75">{status}</p>
    <section className="mt-5 space-y-4">
      ...
      <Panel title="Location"><p className="text-sm font-medium text-white/85">{locationLabel}</p><p className="mt-1 text-xs uppercase tracking-[.16em] text-cyan-200/60">{locationSource}</p><p className="mt-2 text-sm text-white/55">Do not assume the phone and display are in the same place.</p><div className="mt-3 grid gap-2"><Button onClick={() => void save({ location: null })}>Use account default</Button><Button onClick={choosePhoneLocation}>Use my current phone location</Button><Button disabled={!location} onClick={() => void save({ location })}>Use selected display location</Button></div></Panel>
      <Panel title="Time zone"><p className="text-xs text-white/45">Reported by the kiosk, not inferred from this phone.</p><p className="mt-3 text-sm text-white/80">{reportedTimezone ? `${timezoneLabel(reportedTimezone)} · ${reportedTimezone}` : "Time zone could not be detected"}</p><p className="mt-2 text-sm text-white/55">Current display time<br/><span className="text-lg text-white/85">{displayTime(effectiveTimezone)}</span></p><input value={timezone} onChange={(event) => setTimezone(event.target.value)} placeholder="America/Denver" className="mt-3 w-full rounded-xl border border-white/10 bg-white/[.06] px-3 py-3 text-sm text-white"/><Button disabled={saving || !timezone.trim()} onClick={() => void save({ timezoneOverride: timezone.trim() })}>Save timezone override</Button>{timezone && !effectiveTimezone ? <p className="mt-2 text-xs text-rose-200">Time zone could not be detected</p> : null}</Panel>
      ...
      <Panel title="Your Cosmic is ready"><div className="grid grid-cols-2 gap-2"><Metric label="Display" value={reported ? `${reported.viewportWidth} × ${reported.viewportHeight}` : "Waiting"}/><Metric label="UI size" value={`${current.uiScale}`}/><Metric label="Location" value={location ? locationLabel : "Account default"}/><Metric label="Timezone" value={effectiveTimezone ?? "Manual setup needed"}/><Metric label="Night mode" value={current.nightDimEnabled ? `${current.nightDimStart} – ${current.nightDimEnd}` : "Disabled"}/></div><Button disabled={saving} onClick={async () => { const saved = await save({ setupCompleted: true, setupVersion: CURRENT_KIOSK_SETUP_VERSION, setupPreview: "normal", nightDimPreview: false, deviceName: name.trim() || undefined }); if (saved) { setFinished(true); window.setTimeout(() => { window.close(); window.setTimeout(() => router.replace("/activate/setup/complete"), 250); }, 1500); } }}>Finish setup</Button></Panel>
    </section></div></main>;
}
