...
export default function ActivateView({ initialCode }: { initialCode: string }) {
  ...
  async function submit(event: FormEvent) { event.preventDefault(); setBusy(true); setError(null); try { const response = await fetch("/api/devices/pair/approve", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ userCode: code }), credentials: "include", cache: "no-store" }); const body = await response.json() as { error?: string; deviceId?: string }; if (!response.ok) throw new Error(body.error ?? "Code is invalid or expired."); if (!body.deviceId) throw new Error("The display was approved, but its setup identity was not returned."); router.push(`/activate/setup?deviceId=${encodeURIComponent(body.deviceId)}`); } catch (caught) { setState("error"); setError(caught instanceof Error ? caught.message : "Approval failed."); } finally { setBusy(false); } }
