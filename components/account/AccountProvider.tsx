...
interface AccountState { loading: boolean; account: CosmicAccount | null; expiresAt: string | null; sessionType: "user" | "device" | null; deviceId: string | null; authenticated: boolean; isAdmin: boolean; refresh(): Promise<void>; signOut(): Promise<void>; }
...
export function AccountProvider({ children }: { children: React.ReactNode }) {
  ...
  const syncSession = useCallback(async (showLoading: boolean) => {
    ...
    try {
      ...
      setAccount(null); setExpiresAt(null); setSessionType(null); setDeviceId(null); setIsAdmin(false); setActiveCosmicScope("local"); window.dispatchEvent(new CustomEvent("cosmic:auth-lost"));
    } finally { if (showLoading) setLoading(false); }
  }, []);

  const refresh = useCallback(async () => {
    setAccount(null); setExpiresAt(null); setSessionType(null); setDeviceId(null); setIsAdmin(false); setActiveCosmicScope("local");
    await syncSession(true);
  }, [syncSession]);

  const signOut = useCallback(async () => {
    setLoading(true); setAccount(null); setExpiresAt(null); setSessionType(null); setDeviceId(null); setIsAdmin(false); setActiveCosmicScope("local");
  ...
  const value = useMemo(() => ({ loading, account, expiresAt, sessionType, deviceId, isAdmin, authenticated: Boolean(account), refresh, signOut }), [account, deviceId, expiresAt, isAdmin, loading, refresh, sessionType, signOut]);
