...
export default function KioskMusicWidget({ music }: { music: ReturnType<typeof useMusic> }) {
  ...
  const [debugVisible] = useState(() => process.env.NODE_ENV !== "production" && typeof window !== "undefined" && new URLSearchParams(window.location.search).get("cosmic-music-debug") === "1");
  ...
  useEffect(() => { if (!debugVisible) return; const measure = () => { const rect = compositionRef.current?.getBoundingClientRect(); if (!rect) return; const viewportHeight = window.innerHeight; console.info(`[kiosk-music-bounds] viewportHeight=${viewportHeight} contentTop=${Math.round(rect.top)} contentHeight=${Math.round(rect.height)} contentBottom=${Math.round(rect.bottom)} contentCenterY=${Math.round(rect.top + rect.height / 2)} viewportCenterY=${Math.round(viewportHeight / 2)} centerOffset=${Math.round(rect.top + rect.height / 2 - viewportHeight / 2)}`); }; measure(); window.addEventListener("resize", measure); return () => window.removeEventListener("resize", measure); }, [debugVisible, hasTrack, track?.id]);
  return <Widget accent="music" className="kiosk-music-widget" contentPadding={false} hover={false} imageOpacity={1} imageBlur={0}>
    <div className="kiosk-music-scene relative flex h-full min-h-0 flex-col overflow-hidden text-white" data-music-provider={provider?.id ?? "none"}>
      ...
      {debugVisible ? <div className="kiosk-music-debug absolute bottom-2 left-2 z-20 rounded bg-black/75 px-2 py-1 font-mono text-[9px] leading-4 text-white/80">MUSIC DEBUG<br />API: {music.debug?.requestId ?? "—"} · TRACK: {music.debug?.trackIdSuffix ?? "none"}<br />RENDER: {track?.id?.slice(-4) ?? "none"}</div> : null}
    </div>
  </Widget>;
}
