...
export default function GarageWidget() {
  ...
  if (presentation === "kiosk") return <KioskSceneFrame scene="garage" eyebrow="COSMIC • GARAGE" title={loading ? "Checking vehicle." : selectedVehicle?.nickname ?? "No vehicle connected."} subtitle={selectedVehicle ? `${selectedVehicle.currentMileage.toLocaleString()} mi · ${selectedVehicle.status}` : "Vehicle command center"}><div className="kiosk-native-scene-details">{priority && <span>{priority}</span>}{selectedVehicle && <span>{maintenance.length} maintenance due</span>}</div></KioskSceneFrame>;
