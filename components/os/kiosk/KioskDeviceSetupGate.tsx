...
function KioskSetupPreview({ profile, display }: { profile: KioskDeviceProfile; display: KioskDisplayProfile }) {
  ...
  return <div className="kiosk-setup-preview fixed inset-0 overflow-hidden bg-[#030511] text-white" data-kiosk-setup-preview={preview} data-kiosk-preview={PreviewWidget ? "true" : "false"} data-night-preview={profile.nightDimPreview}>
    {preview === "fit" ? <div className="pointer-events-none fixed inset-0 border border-cyan-200/75"><span className="absolute left-0 top-0 h-5 w-5 border-l-2 border-t-2 border-cyan-200"/><span className="absolute right-0 top-0 h-5 w-5 border-r-2 border-t-2 border-cyan-200"/><span className="absolute bottom-0 left-0 h-5 w-5 border-b-2 border-l-2 border-cyan-200"/><span className="absolute bottom-0 right-0 h-5 w-5 border-b-2 border-r-2 border-cyan-200"/><span className="absolute left-1/2 top-1/2 size-3 -translate-x-1/2 -translate-y-1/2 rounded-full bg-cyan-200"/></div> : null}
    {PreviewWidget ? <div className="relative z-10 h-full w-full"><DashboardReadinessProvider criticalWidgetIds={[]}><WidgetProvider size="large" presentation="kiosk"><PreviewWidget /></WidgetProvider></DashboardReadinessProvider><p className="pointer-events-none absolute inset-x-0 bottom-4 z-30 text-center text-xs uppercase tracking-[.2em] text-white/45">Preview · Continue setup on your phone</p></div> : <div className="relative z-10 grid h-full place-items-center p-6 text-center"><div><p className="text-xs uppercase tracking-[.35em] text-cyan-200/70">Cosmic OS</p><h1 className="mt-4 text-3xl font-semibold sm:text-5xl">{profile.deviceName ?? "This Cosmic Display"}</h1><p className="mt-3 text-sm uppercase tracking-[.24em] text-white/55">{preview === "fit" ? "Screen fit test" : "Setting up this display"}</p><p className="mt-5 text-sm text-white/45">Continue setup on your phone.</p><p className="mt-3 text-xs text-white/30">{display.viewportWidth} × {display.viewportHeight} · {display.density} · {display.orientation}</p></div></div>}
