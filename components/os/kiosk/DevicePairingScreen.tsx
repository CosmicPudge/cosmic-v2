...
type HelperHandoff = { state?: "ready" | "reconnecting" | "unauthorized" | "needs_provisioning" | "identity_recovery" | "recovery_required"; handoffToken?: string; activationUrl?: string; publicNumber?: string; pairingRequired?: boolean };
  ...
  useEffect(() => {
    ...
    async function start() {
      try {
        ...
        if (helper && helper.ok && helperBody?.state === "needs_provisioning" && helperBody.pairingRequired) { /* Continue into the normal pairing request below. */ }
        else if (helper && !helper.ok && helperBody?.state === "needs_provisioning" && !helperBody.pairingRequired) { setEnrollment({ activationUrl: helperBody.activationUrl ?? "", publicNumber: helperBody.publicNumber }); return; }
        if (helper && !helper.ok && helperBody?.state === "recovery_required") { setEnrollment({ activationUrl: helperBody.activationUrl ?? "", publicNumber: helperBody.publicNumber }); return; }
