...
export default function CosmicPlusPage() {
  ...
  const { data: billing, loading, error: billingError, refresh } = useBillingStatus();
  ...
  const paymentNeedsAttention = status === "past_due" || status === "unpaid" || status === "incomplete";
  const hasExistingBilling = Boolean(billing.subscription || billing.customerExists);
  const checkoutConfigured = billing.configuration.checkoutConfigured;
  const recoveryNeedsPortal = hasExistingBilling && (status === "past_due" || status === "unpaid" || status === "incomplete");
  ...
  useEffect(() => {
    ...
    const poll = async () => { if (cancelled) return; await refresh(); await refreshEntitlements(); window.dispatchEvent(new Event("cosmic:entitlements-updated")); attempts += 1; if (attempts >= 5) { setCheckoutPending(false); return; } window.setTimeout(() => void poll(), 2000); };
