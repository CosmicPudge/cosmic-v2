...
import { getNextClassToday, getOverdueAssignments, getSchoolToday, getUpcomingAssignments } from "@/services/school/semesterConsumers";
...
function activeAssignments(items: SchoolPlanningAssignment[]) {
  return items.filter(isAssignmentActiveForPlanning);
}
function sortAssignments(a: SchoolPlanningAssignment, b: SchoolPlanningAssignment) {
  const aDone = !isAssignmentActiveForPlanning(a);
  const bDone = !isAssignmentActiveForPlanning(b);
...
function AssignmentRow({ item }: { item: SchoolPlanningAssignment }) {
  const overdue = item.dueAt && item.dueAt < new Date() && isAssignmentActiveForPlanning(item);
  const complete = !isAssignmentActiveForPlanning(item);
...
export function AcademicCommandCenter() {
  ...
  const todayData = getSchoolToday(courses, term, assignments, snapshot?.events ?? [], today);
  const dueToday = todayData.assignments;
  const overdue = getOverdueAssignments(assignments, today);
  const upcoming = getUpcomingAssignments(assignments, today);
  const nextClass = getNextClassToday(todayData.meetings, today);
  const [query, setQuery] = useState("");
  const filtered = activeAssignments(assignments).filter((item) => `${item.title} ${item.courseName ?? ""} ${sourceLabel(item.sourceType)}`.toLowerCase().includes(query.toLowerCase())).slice(0, 12);
  ...
  return <div className="space-y-6">
    ...
    <section className="grid gap-3 sm:grid-cols-3"><Metric label="Due today" value={dueToday.length} tone="text-amber-100" /><Metric label="Overdue" value={overdue.length} tone="text-rose-200" /><Metric label="Next 7 days" value={upcoming.length} tone="text-sky-100" /></section>
    <section className="grid gap-4 xl:grid-cols-[1.35fr_.85fr]">
      <div className={`${panel} overflow-hidden`}><div className="flex items-center justify-between border-b border-white/[0.08] px-4 py-4"><div><p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-sky-200/55">Today</p><h2 className="mt-1 text-xl font-semibold text-white">What needs your attention</h2></div><Sparkles className="size-5 text-sky-200/55" /></div><div className="divide-y divide-white/[0.06] px-4 py-3">{todayData.meetings.map((item) => <div key={item.id} className="flex items-center gap-3 py-2"><Clock3 className="size-4 text-sky-200/70" /><span className="min-w-0 flex-1 truncate text-sm text-white/80">{item.course.name}</span><span className="text-xs text-white/40">{item.meeting.startTime}–{item.meeting.endTime}</span></div>)}{dueToday.slice(0, 4).map((item) => <div key={item.id} className="flex items-center gap-3 py-2"><span className="size-2 rounded-full bg-amber-200" /><span className="min-w-0 flex-1 truncate text-sm text-white/80">{item.title}</span><span className="text-xs text-white/40">{item.courseName ?? "Unknown course"}</span></div>)}{todayData.events.slice(0, 3).map((item) => <div key={`${item.source}:${item.id}`} className="flex items-center gap-3 py-2"><CalendarDays className="size-4 text-violet-200/70" /><span className="min-w-0 flex-1 truncate text-sm text-white/80">{item.title}</span><span className="text-xs text-white/40">Academic event</span></div>)}{overdue.slice(0, 3).map((item) => <div key={`overdue-${item.id}`} className="flex items-center gap-3 py-2"><span className="size-2 rounded-full bg-rose-300" /><span className="min-w-0 flex-1 truncate text-sm text-white/80">{item.title}</span><span className="text-xs text-rose-200/75">Overdue</span></div>)}{!todayData.meetings.length && !dueToday.length && !todayData.events.length && !overdue.length && <p className="py-4 text-sm text-white/45">Nothing scheduled or due today.</p>}</div></div>
      <div className={`${panel} p-5`}><p className="text-[10px] font-semibold tracking-[0.2em] text-sky-200/55">Next class</p>{nextClass ? <><h2 className="mt-3 text-lg font-semibold text-white">{courseName(nextClass.course)}</h2><p className="mt-2 flex items-center gap-2 text-sm text-white/55"><Clock3 className="size-4" /> {nextClass.meeting.startTime}–{nextClass.meeting.endTime}</p>{(nextClass.meeting.location ?? nextClass.course.location) && <p className="mt-2 flex items-center gap-2 text-sm text-white/55"><MapPin className="size-4" /> {nextClass.meeting.location ?? nextClass.course.location}</p>}</> : <p className="mt-3 text-sm text-white/45">No more classes today.</p>}<Link href="/school/schedule" className="mt-5 inline-flex items-center gap-1 text-sm text-sky-100/75 hover:text-white">Open schedule <ChevronRight className="size-4" /></Link></div>
    </section>
...
export function SchoolAssignmentsCommand() {
  ...
  const now = new Date();
  const shown = items.filter((item) => { const done = !isAssignmentActiveForPlanning(item); const week = item.dueAt && item.dueAt.getTime() <= now.getTime() + 7 * 86_400_000; const match = `${item.title} ${item.courseName ?? ""} ${sourceLabel(item.sourceType)}`.toLowerCase().includes(query.toLowerCase()); if (!match || (courseFilter !== "all" && item.courseName !== courseFilter) || (sourceFilter !== "all" && item.sourceType !== sourceFilter)) return false; return view === "All" || view === "Completed" && done || view === "Overdue" && !done && !!item.dueAt && item.dueAt < now || view === "Due soon" && !done && !!item.dueAt && item.dueAt >= now && week || view === "No due date" && !done && !item.dueAt; });
