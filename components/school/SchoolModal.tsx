...
export function SchoolModal({ title, children, onClose }: { title: string; children: ReactNode; onClose: () => void }) {
  return <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/60 p-[max(1rem,env(safe-area-inset-top))_1rem_max(1rem,env(safe-area-inset-bottom))] backdrop-blur-sm sm:items-center" onMouseDown={onClose}><section className="flex max-h-[calc(100dvh-2rem)] min-h-0 w-full max-w-2xl flex-col overflow-hidden rounded-3xl border border-white/15 bg-[#0d111d]/95 p-6 shadow-2xl" onMouseDown={(event) => event.stopPropagation()} role="dialog" aria-modal="true" aria-label={title}><header className="flex shrink-0 items-center justify-between gap-4"><h2 className="text-xl font-semibold text-white">{title}</h2><button type="button" onClick={onClose} className="rounded-xl border border-white/10 px-3 py-2 text-sm text-white/60 hover:bg-white/10">Close</button></header><div className="mt-5 min-h-0 flex-1 overflow-y-auto overscroll-contain pr-1">{children}</div></section></div>;
}
