...
export const schoolAudioTranscripts = pgTable("school_audio_transcripts", {
  id: text("id").primaryKey(), userId: text("user_id").notNull().references(() => users.id, { onDelete: "cascade" }), sourceId: text("source_id").notNull().references(() => schoolSources.id, { onDelete: "cascade" }), assetId: text("asset_id").references(() => schoolAssets.id, { onDelete: "set null" }), courseId: text("course_id"), sourceType: text("source_type").notNull().default("cosmic_transcription"), sourceLabel: text("source_label"), transcript: text("transcript"), segments: jsonb("segments"), provider: text("provider"), model: text("model"), providerAttempts: jsonb("provider_attempts"), organizedContent: text("organized_content"), organizedTopics: jsonb("organized_topics"), title: text("title"), status: text("status").notNull().default("uploaded"), audioCleanupStatus: text("audio_cleanup_status").notNull().default("not_applicable"), audioDeletedAt: timestamp("audio_deleted_at", { withTimezone: true }), processingError: text("processing_error"), createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(), updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow(),
}, (table) => [index("school_audio_transcripts_user_index").on(table.userId), uniqueIndex("school_audio_transcripts_source_unique").on(table.sourceId), check("school_audio_transcripts_status_check", sql`${table.status} in ('uploaded', 'transcribing', 'transcribed', 'organizing', 'ready-for-review', 'approved', 'failed')`), check("school_audio_cleanup_status_check", sql`${table.audioCleanupStatus} in ('not_applicable', 'uploaded', 'processing', 'transcribed', 'deletion_pending', 'deleted')`)]);
...
export const schoolStudySets = pgTable("school_study_sets", {
  id: text("id").primaryKey(), accountId: text("account_id").notNull().references(() => users.id, { onDelete: "cascade" }), courseId: text("course_id"), title: text("title").notNull(), description: text("description"), provenance: jsonb("provenance"), createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(), updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow(),
}, (table) => [index("school_study_sets_account_index").on(table.accountId), index("school_study_sets_course_index").on(table.accountId, table.courseId)]);

export const schoolFlashcards = pgTable("school_flashcards", {
  id: text("id").primaryKey(), setId: text("set_id").notNull().references(() => schoolStudySets.id, { onDelete: "cascade" }), accountId: text("account_id").notNull().references(() => users.id, { onDelete: "cascade" }), front: text("front").notNull(), back: text("back").notNull(), notes: text("notes"), reviewCount: integer("review_count").notNull().default(0), intervalDays: integer("interval_days").notNull().default(0), lastReviewedAt: timestamp("last_reviewed_at", { withTimezone: true }), nextReviewAt: timestamp("next_review_at", { withTimezone: true }), createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(), updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow(),
}, (table) => [index("school_flashcards_set_index").on(table.setId), index("school_flashcards_account_review_index").on(table.accountId, table.nextReviewAt)]);

export const schoolResources = pgTable("school_resources", {
  id: text("id").primaryKey(), accountId: text("account_id").notNull().references(() => users.id, { onDelete: "cascade" }), courseId: text("course_id"), title: text("title").notNull(), url: text("url"), reference: text("reference"), description: text("description"), createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(), updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow(),
}, (table) => [index("school_resources_account_index").on(table.accountId), index("school_resources_course_index").on(table.accountId, table.courseId), check("school_resources_url_or_reference_check", sql`${table.url} is not null or ${table.reference} is not null`)]);
...
export const schoolCanvasCalendarEvents = pgTable("school_canvas_calendar_events", {
  id: text("id").primaryKey(), userId: text("user_id").notNull().references(() => users.id, { onDelete: "cascade" }), sourceId: text("source_id").notNull(), uid: text("uid").notNull(), recurrenceId: text("recurrence_id"), title: text("title").notNull(), description: text("description"), location: text("location"), url: text("url"), eventType: text("event_type").notNull(), courseId: text("course_id"), courseMatchReason: text("course_match_reason"), startAt: timestamp("start_at", { withTimezone: true }), endAt: timestamp("end_at", { withTimezone: true }), status: text("status"), sequence: integer("sequence"), lastModified: text("last_modified"), dtstamp: text("dtstamp"), firstSeenAt: timestamp("first_seen_at", { withTimezone: true }).notNull().defaultNow(), lastSeenAt: timestamp("last_seen_at", { withTimezone: true }).notNull().defaultNow(), presence: text("presence").notNull().default("present"), linkedAssignmentId: text("linked_assignment_id"), updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow(),
}, (table) => [index("school_canvas_calendar_events_user_index").on(table.userId), index("school_canvas_calendar_events_source_index").on(table.userId, table.sourceId), uniqueIndex("school_canvas_calendar_events_identity_unique").on(table.userId, table.id), check("school_canvas_calendar_events_presence_check", sql`${table.presence} in ('present', 'missing', 'stale', 'archived')`)]);
