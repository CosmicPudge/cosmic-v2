...
export const schoolAudioTranscripts = pgTable("school_audio_transcripts", {
  id: text("id").primaryKey(), userId: text("user_id").notNull().references(() => users.id, { onDelete: "cascade" }), sourceId: text("source_id").notNull().references(() => schoolSources.id, { onDelete: "cascade" }), assetId: text("asset_id").notNull().references(() => schoolAssets.id, { onDelete: "cascade" }), courseId: text("course_id"), transcript: text("transcript"), segments: jsonb("segments"), provider: text("provider"), model: text("model"), providerAttempts: jsonb("provider_attempts"), organizedContent: text("organized_content"), organizedTopics: jsonb("organized_topics"), title: text("title"), status: text("status").notNull().default("uploaded"), processingError: text("processing_error"), createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(), updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow(),
}, (table) => [index("school_audio_transcripts_user_index").on(table.userId), uniqueIndex("school_audio_transcripts_source_unique").on(table.sourceId), check("school_audio_transcripts_status_check", sql`${table.status} in ('uploaded', 'transcribing', 'transcribed', 'organizing', 'ready-for-review', 'approved', 'failed')`)]);
