...
export function hydrateSchoolPlanningAssignments(value: unknown): SchoolPlanningAssignment[] {
  ...
  for (const item of value) {
    ...
    parsed.push({ ...(raw as unknown as SchoolPlanningAssignment), createdAt, updatedAt, ...(dueAt ? { dueAt } : {}), ...(availableAt ? { availableAt } : {}), ...(lockAt ? { lockAt } : {}), ...(lastSyncedAt ? { lastSyncedAt } : {}), ...(sourceUpdatedAt ? { sourceUpdatedAt } : {}) });
  }
  return parsed;
}
