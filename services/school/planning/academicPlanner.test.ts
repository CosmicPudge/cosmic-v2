...
test("dedupes duplicate Canvas planner representations while preserving generated blocks", () => { const item = assignment("canvas-calendar:event-assignment-5350137", "2026-09-17T23:59:00Z", { sourceType: "canvas-calendar", externalId: "event-assignment-5350137", estimatedMinutes: 90 }); const plan = buildProactivePlan(snapshot([item, { ...item }]), now); assert.equal(plan.assignments.length, 1); assert.equal(new Set(plan.workBlocks.map((block) => `${block.assignmentId}:${block.date.toISOString()}:${block.minutes}`)).size, plan.workBlocks.length); });
