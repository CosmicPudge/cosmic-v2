...
import { SchoolTranscriptionError, validateSchoolAudio, type SchoolAudioMimeType } from "./audio";
import { transcribeWithSchoolRouter } from "./transcriptionRouter";

function safeError(error: unknown) { if (error instanceof SchoolTranscriptionError) return error.code === "transcription_quota" ? "OpenAI transcription limit reached." : ["transcription_rate_limit", "transcription_provider_unavailable", "transcription_timeout"].includes(error.code) ? "Transcription temporarily unavailable." : error.code === "transcription_invalid_audio" ? "The recording could not be validated as audio." : error.code === "transcription_unsupported_audio" ? "This recording format is not supported." : error.code === "transcription_too_large" ? "This recording is too large for the configured transcription providers." : error.code === "transcription_auth_configuration" ? "Transcription is not configured." : "Transcription could not be completed."; return error instanceof Error ? error.message.replace(/\s+/g, " ").slice(0, 240) : "Audio processing failed."; }
