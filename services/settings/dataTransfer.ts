...
function validateDomain(domain: CosmicBackupDomain, value: unknown): DomainSnapshots[typeof domain] | null {
  ...
  if (domain === "garage") {
    ...
    return { version: 1, ...(typeof value.selectedVehicleId === "string" ? { selectedVehicleId: value.selectedVehicleId } : {}), ...(typeof value.selectedCollectionId === "string" ? { selectedCollectionId: value.selectedCollectionId } : {}), vehicles: value.vehicles, collections: Array.isArray(value.collections) ? value.collections : [], maintenance: value.maintenance, services: value.services, issues: value.issues, modifications: value.modifications, expenses: value.expenses, reminders: value.reminders, mileageHistory: value.mileageHistory, fuelRecords: Array.isArray(value.fuelRecords) ? value.fuelRecords : [], documents: Array.isArray(value.documents) ? value.documents : [], connections: Array.isArray(value.connections) ? value.connections : [], diagnosticScans: Array.isArray(value.diagnosticScans) ? value.diagnosticScans : [], telemetrySnapshots: Array.isArray(value.telemetrySnapshots) ? value.telemetrySnapshots : [], repairEstimates: Array.isArray(value.repairEstimates) ? value.repairEstimates : [] } as DomainSnapshots[typeof domain];
  }
  if (domain === "projects") {
