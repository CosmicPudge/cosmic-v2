...
export const databaseAuthRepository: AuthRepository = {
  ...
  async findSession(tokenHash) { const result = await getDatabase().select().from(sessions).innerJoin(users, eq(sessions.userId, users.id)).where(and(eq(sessions.sessionTokenHash, tokenHash), isNull(sessions.revokedAt), gt(sessions.expiresAt, new Date()))).limit(1); const row = result[0]; if (!row || row.users.status === "disabled") return null; const lastUsedAt = new Date(); await getDatabase().update(sessions).set({ lastUsedAt }).where(eq(sessions.id, row.sessions.id)); return session({ ...row.sessions, lastUsedAt }, row.users); },
