...
export async function createSession(accountId: string, userAgent?: string, options?: { sessionType?: "user" | "device"; deviceId?: string; expiresAt?: string }) { const token = randomBytes(32).toString("base64url"); const expiresAt = options?.expiresAt ?? new Date(Date.now() + SESSION_DAYS * 24 * 60 * 60 * 1000).toISOString(); const session = await getAuthRepository().createSession({ userId: accountId, tokenHash: hashSessionToken(token), expiresAt, userAgent, sessionType: options?.sessionType, deviceId: options?.deviceId }); return { token, ...session }; }
export async function getSession(request: Request) { const token = parseSessionCookie(request); if (!token) return null; return getAuthRepository().findSession(hashSessionToken(token)); }
