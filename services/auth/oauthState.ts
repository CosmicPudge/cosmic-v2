...
export function createOAuthState(accountId?: string, returnTo?: string, provider?: string) {
  const encodedReturnTo = returnTo ? Buffer.from(safeReturnUrl(returnTo, "/")).toString("base64url") : "";
  const value = `${randomUUID()}|${accountId ?? "local"}|${Date.now() + TTL_MS}|${encodedReturnTo}|${provider ?? ""}`;
...
export function consumeOAuthState(request: Request, state: string | null, accountId?: string, provider?: string) {
  return readOAuthState(request, state, accountId, provider) !== null;
}
...
function readOAuthState(request: Request, state: string | null, accountId?: string, provider?: string) {
  ...
  try {
    ...
    const [storedState, storedAccount, expires, , storedProvider] = value.split("|");
    return storedState === state && Number(expires) > Date.now() && storedAccount === (accountId ?? "local") && (!provider || storedProvider === provider) ? value : null;
  } catch { return null; }
}
