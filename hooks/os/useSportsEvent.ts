...
export function useSportsEvent(eventId: string) {
  ...
  useVisiblePolling(async () => { try { await refresh(); } catch (reason) { setError(reason instanceof Error ? reason.message : "Sports event data is unavailable."); setLoading(false); } }, polling, { immediate: true, enabled: status !== "final" && status !== "cancelled" && status !== "postponed" });
