...
try {
  ...
  const reusedConsume = await request("/api/devices/pair/status", { method: "POST", body: JSON.stringify({ deviceCode: reusedPairBody.deviceCode }) }); assert.equal(reusedConsume.status, 200); assert.equal((await json(reusedConsume)).initialEnrollmentRequired, true); assert.equal((await sql("select credential_hash from devices where id=$1", [deviceId])).rows[0].credential_hash, null);
  const initialCredential = `initial-${randomBytes(32).toString("base64url")}`; const initialEnroll = await request("/api/devices/pair/initial-enroll", { method: "POST", body: JSON.stringify({ deviceCode: reusedPairBody.deviceCode, bootId, deviceId, publicNumber, credentialHash: sha(initialCredential) }) }); assert.equal(initialEnroll.status, 200); const initialEnrollBody = await json(initialEnroll); assert.equal(initialEnrollBody.enrolled, true); assert.equal("credential" in initialEnrollBody, false); assert.equal((await request("/api/devices/bootstrap", { method: "POST", headers: { Authorization: `Bearer ${initialCredential}` }, body: JSON.stringify({ bootId }) })).status, 200);
  await sql("update devices set credential_hash=null, credential_revoked_at=now() where id=$1", [deviceId]); const ownedWithoutCredential = await request("/api/devices/lifecycle", { method: "POST", body: JSON.stringify({ deviceId, publicNumber }) }); assert.equal(ownedWithoutCredential.status, 200); assert.deepEqual(await json(ownedWithoutCredential), { state: "recovery_required", pairingRequired: false, deviceId, publicNumber });
