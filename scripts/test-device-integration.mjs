...
try {
  ...
  const reusedConsume = await request("/api/devices/pair/status", { method: "POST", body: JSON.stringify({ deviceCode: reusedPairBody.deviceCode }) }); assert.equal(reusedConsume.status, 200); assert.equal((await sql("select credential_hash from devices where id=$1", [deviceId])).rows[0].credential_hash, null);
